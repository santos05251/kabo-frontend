...
class FrequencyModal extends React.Component {
  ...
  handleChange = (event) => {
    ...
    if (key && this.props.user) {
      ...
      this.setState({
        changed: {
          ...this.state.changed,
          [key]: `${event.target.value}` !== `${originalValue}`,
        },
      });
    }
  };
  ...
  render() {
    ...
    const disabled =
      !changed.starting_date && !changed.how_often && !changed.amount_of_food;

    const SelectOptions = (array) =>
      array.map((delayOption, i) => (
        <option className="w-full" key={i} value={delayOption.value}>
          {delayOption.label}{" "}
        </option>
      ));
      ...
      <div className="py-8 px-5 relative border-r border-l rounded-b-xl border-b border-gray-300">
        ...
        <form onSubmit={this.handleSubmit}>
          ...
          <button
            ...
            } text-white rounded-xl font-semibold py-2.5 px-6 mt-8`}
          >
            Save Changes
          </button>
        </form>
