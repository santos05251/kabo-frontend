...
const FoodCard = ({ type, food, icons, kibble, selectCookedFood, selectedCookedRecipes, selectKibbleRecipe, selected, selectedLength }) => {
  ...
  const selectedText =
    "bg-green-700 border border-green-700 hover:border-transparent focus:outline-none text-white text-sm md:text-base font-bold p-1 md:py-2 md:px-5 w-4/5 rounded-xl mt-2 md:mt-0";
  const unSelectedText =
    "bg-transparent border border-green-700 hover:border-transparent focus:outline-none hover:bg-green-700 text-primary hover:text-white font-bold text-sm md:text-base w-4/5 p-1 md:py-2 md:px-5 rounded-xl border-green  mt-2 md:mt-0";
  return (
    <div className="rounded-xl md:h-40 w-full mb-4 flex md:flex-row flex-col overflow-hidden">
      ...
      >
        <div className="font-cooper md:text-xl text-sm text-black text-center">{food.name}</div>
        <div
          onClick={() => {
            openDetails(true);
          }}
          className="text-primary font-bold text-sm md:text-base mt-2 md:mt-0 font-messina cursor-pointer"
        >
          See Details
        </div>
        {type === "kibble" ? (
          ...
          <button className={selected ? selectedText : unSelectedText} onClick={() => selectCookedFood(food)} disabled={selectedLength >= 2 && !selected}>
            {selected ? "Recipe Added" : "Add Recipe"}
          </button>
        )}
        <OrderItemModal
